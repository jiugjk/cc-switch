...
| `app` | はい | アプリタイプ：`claude` / `codex` / `gemini` / `grokbuild` / `opencode` / `openclaw` / `hermes` |
| `name` | はい | 名前 |
...
| パラメータ | 必須 | 説明 |
|------|------|------|
| `apps` | はい | アプリリスト（カンマ区切り。`claude` / `codex` / `gemini` / `grokbuild` / `opencode` / `openclaw` / `hermes` に対応。例：`claude,codex,gemini,opencode`） |
| `config` | はい | MCP サーバー設定（JSON 形式） |
| `enabled` | いいえ | 明示的な `true` の場合のみ、選択したアプリへ直ちにサーバー設定を書き込みます。省略時または `false` の場合は無効状態でインポートされます（デフォルト：`false`） |
