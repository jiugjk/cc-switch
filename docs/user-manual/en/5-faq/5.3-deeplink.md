...
| `app` | Yes | App type: `claude` / `codex` / `gemini` / `grokbuild` / `opencode` / `openclaw` / `hermes` |
| `name` | Yes | Name |
...
| Parameter | Required | Description |
|-----------|----------|-------------|
| `apps` | Yes | App list (comma-separated; supports `claude` / `codex` / `gemini` / `grokbuild` / `opencode` / `openclaw` / `hermes`, e.g., `claude,codex,gemini,opencode`) |
| `config` | Yes | MCP server configuration (JSON format) |
| `enabled` | No | Only an explicit `true` immediately writes the servers to the selected apps; omitted or `false` imports them disabled (default: `false`) |
...
```
ccswitch://v1/import?resource=mcp&apps=claude,codex&config=%7B%22command%22%3A%22uvx%22%2C%22args%22%3A%5B%22mcp-server-fetch%22%5D%7D&name=mcp-fetch&enabled=true
```
...
```
ccswitch://v1/import?resource=mcp&name=mcp-fetch&apps=claude,codex,gemini&config=%7B%22command%22%3A%22uvx%22%2C%22args%22%3A%5B%22mcp-server-fetch%22%5D%7D&enabled=true
```
