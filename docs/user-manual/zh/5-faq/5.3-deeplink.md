...
| `app` | 是 | 应用类型：`claude` / `codex` / `gemini` / `grokbuild` / `opencode` / `openclaw` / `hermes` |
| `name` | 是 | 名称 |
...
| 参数 | 必填 | 说明 |
|------|------|------|
| `apps` | 是 | 应用列表（逗号分隔；支持 `claude` / `codex` / `gemini` / `grokbuild` / `opencode` / `openclaw` / `hermes`，如 `claude,codex,gemini,opencode`） |
| `config` | 是 | MCP 服务器配置（JSON 格式） |
| `enabled` | 否 | 仅显式设为 `true` 时才会立即写入所选应用；省略或设为 `false` 时以禁用状态导入（默认：`false`） |
