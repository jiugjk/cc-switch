...
`サードパーティ切替時に公式ログインを保持` をオフにすると、サードパーティプロバイダー切り替えは旧バージョン互換の動作になり、`auth.json` が再度書き込まれる可能性があります。公式リモート操作と公式プラグインを長期的に保持したい場合は、このスイッチをオンのままにすることをおすすめします。スイッチがオフでも安全弁が 1 つ残ります。切り替え先プロバイダーのスナップショットが（サードパーティ API キーではなく）同じアカウントの ChatGPT OAuth トークンを保存している場合、CC Switch はより新しいライブログインをダウングレードせず、`last_refresh` が新しい live `auth.json` のトークンを維持します。API キープロバイダーは上記の上書き動作のままです。
