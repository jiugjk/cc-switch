...
如果关闭 `切换第三方时保留官方登录`，第三方供应商切换会沿用兼容旧版本的行为，可能重新写入 `auth.json`。如果你的目标是长期保留官方远程操作和官方插件，建议保持该开关开启。即使开关关闭，也保留一条安全底线：当切入的供应商快照存的是同一账号的 ChatGPT OAuth 令牌（而非第三方 API Key）时，CC Switch 不会用旧令牌覆盖更新的在线登录——`last_refresh` 更新的 live `auth.json` 令牌会被保留；API Key 供应商仍是上述覆盖行为。
