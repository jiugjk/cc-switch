...
> 注意：設定の「新規 Codex 設定に requires_openai_auth を含める」トグルはこの注入には**影響しません**。あのトグルは新しいサードパーティ設定の生成時デフォルトを変えるだけです。ここではこのフィールドが機能的に必須であり（auth.json の ChatGPT ログインをルーティングします）、剥離の往復が注入テーブルの正確な形状に依存するため、この機能が有効な間は常に書き込まれます。
