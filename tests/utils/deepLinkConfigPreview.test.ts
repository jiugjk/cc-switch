...
`;
  ...
  it("previews generic app JSON and recursively masks secrets", () => {
    const preview = parseDeepLinkConfigPreview({
      app: "hermes",
      config: encodeBase64(
        JSON.stringify({
    ...
    expect(preview?.configText).toContain("https://hermes.example.com");
    expect(preview?.configText).not.toContain("secret-hermes-key");
    expect(preview?.configText).not.toContain("Bearer hidden");
  });
});
